...
def print_cmd_line_worker(item, fmt, cmd):
    ...
    if not BuildLogFile:
        BuildLogFile = open('paysharesd-build.log', 'w')
...
COMPILED_FILES.extend([
	...
	'src/ripple_data/crypto/PaysharesPrivateKey.cpp',
	'src/ripple_data/crypto/PaysharesPublicKey.cpp',
...
https://wiki.payshares.org/Ubuntu_Build_Instructions#Ubuntu_versions_older_than_13.10_:_Install_gcc_4.8'''

if not USING_CLANG:
    if (int(GCC_VERSION[0]) == 4 and int(GCC_VERSION[1]) < 8):
        print "\n\033[91mTo compile paysharesd using GCC you need version 4.8.1 or later.\033[0m\n"
...
PaysharesRepository = False

# Determine if we're building against the main payshares repo or a developer repo
if TravisBuild:
    Slug = os.environ.get('TRAVIS_REPO_SLUG', '')

    if (Slug.find ("payshares/") == 0):
        PaysharesRepository = True

if TravisBuild:
    env.Append(CFLAGS = ['-DTRAVIS_CI_BUILD'])
    env.Append(CXXFLAGS = ['-DTRAVIS_CI_BUILD'])

if PaysharesRepository:
...
if TravisBuild:
    ...
    if (Slug.find ("payshares/") == 0):
        print_nv_pair ("Build", "Travis - Payshares Master Repository")
    else:
        print_nv_pair ("Build", "Travis - Payshares Developer Fork")
...
paysharesd = env.Program('build/paysharesd', OBJECT_FILES)

tags    = env.CTags('tags', TAG_SRCS)

Default(paysharesd, tags)
