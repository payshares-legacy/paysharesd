...
public:
    ...
    void initSqliteDb (int index)
    {
        ...
        case 4: mWorkingLedgerDB.reset(openDatabaseCon ("workingLedger.db", payshares::LedgerDatabase::getSQLInit())); break;
        };
    }
    ...
    void setup ()
    {
        ...
        Log (lsWARNING) << "Starting up paysharesd version " << BuildInfo::getFullVersionString ();
        ...
        payshares::gLedgerMaster =std::make_shared<payshares::LedgerMaster>();
        ...
        payshares::gLedgerMaster->loadLastKnownCLF();
        ...
        if (!ledgerToLoad.empty() && !loadOldLedger (ledgerToLoad, getConfig ().START_UP == Config::REPLAY, payshares::gLedgerMaster->getCurrentCLF()))
        {
    ...
    void prepareValidators ()
    {
#if 1
        {
            ...
            m_validators->addStrings ("paysharesd.cfg", strings);
        }
#endif
...
{
    // New stuff.
    RippleAddress   rootSeedMaster      = RippleAddress::createSeedGeneric ("paysharesmaster");
    ...
    // Treefunder : let's rip this
    RippleAddress naSeed;
    naSeed.setSeedGeneric("paysharesmaster");
    RippleAddress naNodePrivate = RippleAddress::createNodePrivate(naSeed);
    m_journal.info << "Root human node private: " << naNodePrivate.humanNodePrivate();

    {
        ...
        payshares::gLedgerMaster->ensureSync(secondLedger, false);
    }
}
...
bool ApplicationImp::loadOldLedger (const std::string& l, bool bReplay, CanonicalLedgerForm::pointer ledgerInDB)
{
    try
    {
        ...
        if (!getConfig().DONT_WALK_LOADED_LEDGER && !payshares::gLedgerMaster->ensureSync(loadLedger, false))
        {
