...
public:
    ...
    // DEPRECATED
    boost::filesystem::path     VALIDATORS_FILE;        // As specified in paysharesd.cfg.
    ...
    */
    ...
    /** List of Validators entries from paysharesd.cfg */
...
public:
    ...
    */
    ...
    /** Parameters for the main NodeStore database.
        ...
        The 'type' and 'path' keys are required, see paysharesd-example.cfg
...
public:
    ...
    std::vector<std::string>    IPS;                    // Peer IPs from paysharesd.cfg.
    std::vector<std::string>    IPS_FIXED;              // Fixed Peer IPs from paysharesd.cfg.
    std::vector<std::string>    SNTP_SERVERS;           // SNTP servers from paysharesd.cfg.
